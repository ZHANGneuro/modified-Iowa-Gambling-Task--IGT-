...
% 
% To perform the script, type 'igt(Subject_ID)', ex. igt(1),
% igt(2)....igt(n). for quiting, type 'ESC'. 
% 
% 
% 
...
function igt(subject_ID)
    ...
    for deck_jump = 1:120
        ...
        while GetSecs - startTime < dur_deck  
            ...
            WaitSecs(0.001);
            responseTime = secs;
            key_selected = find(keycode_al);
            
            if key_selected == KbName('escape')
                Screen('CloseAll');
                return;
            end

            if ~isempty(key_selected)
                if  key_selected == KbName('a') && isempty(c1)
                    ...
                    Screen('DrawText', wPtr, net_income, w/2-3*w/16-2*deck_width+deck_width/10, h/5+deck_height+4*h/50,[0 255 0])
                    else 
                    Screen('DrawText', wPtr, net_income, w/2-3*w/16-2*deck_width+deck_width/10, h/5+deck_height+4*h/50,[255 0 0])
                    end
                    ...
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                    Screen('DrawTexture', wPtr, t2, [], deck_A);
                    Screen('DrawTexture', wPtr, t1, [], deck_B);
                    Screen('DrawTexture', wPtr, t1, [], deck_C);
                    Screen('DrawTexture', wPtr, t1, [], deck_D);
                    ...
                    while 1
                        [secs, temp_keycode, deltaSecs] = KbWait([], 2);
                        WaitSecs(0.1);
                        if temp_keycode(KbName('SPACE'))&size(find(temp_keycode), 2)==1
                            break;
                        end
                        if temp_keycode(KbName('escape'))
                            Screen('CloseAll');
                            return;
                        end
                    end
                ...
                elseif   key_selected == KbName('a') && isempty(c2)
                    ...
                    Screen('DrawText', wPtr, 'Deck D', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+h/50,0)
                    if net_change(deck_jump+1,1) > 1
                        Screen('DrawText', wPtr, net_income, w/2-w/16-deck_width+deck_width/10, h/5+deck_height+4*h/50,[0 255 0])
                    else
                        Screen('DrawText', wPtr, net_income,w/2-w/16-deck_width+deck_width/10, h/5+deck_height+4*h/50,[255 0 0])
                    end
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Current_total_show, 'center', 12*h/20, 0);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Previous_total_show, 'center', 13*h/20, gray);
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                ...
                elseif key_selected == KbName('a') && isempty(c3)
                    ...
                    Screen('DrawText', wPtr, 'Deck D', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+h/50,0)
                    if net_change(deck_jump+1,1) > 1
                        Screen('DrawText', wPtr, net_income,w/2+w/16+deck_width/10, h/5+deck_height+4*h/50,[0 255 0])
                    else
                        Screen('DrawText', wPtr, net_income,w/2+w/16+deck_width/10, h/5+deck_height+4*h/50,[255 0 0])
                    end
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Current_total_show, 'center', 12*h/20, 0);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Previous_total_show, 'center', 13*h/20, gray);
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                ...
                elseif key_selected == KbName('a') && isempty(c4)
                    ...
                    Screen('DrawText', wPtr, 'Deck D', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+h/50,0)
                    if net_change(deck_jump+1,1) > 1
                        Screen('DrawText', wPtr, net_income,w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+4*h/50,[0 255 0])
                    else
                        Screen('DrawText', wPtr, net_income,w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+4*h/50,[255 0 0])
                    end
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Current_total_show, 'center', 12*h/20, 0);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Previous_total_show, 'center', 13*h/20, gray);
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                ...
                elseif  key_selected == KbName('l') && isempty(c1)
                    ...
                    Screen('DrawText', wPtr, 'Deck C', w/2+w/16+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'Deck D', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'PASS', w/2-3*w/16-2*deck_width+deck_width/10, h/5+deck_height+4*h/50, [255 0 0]);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Current_total_show, 'center', 12*h/20, 0);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Previous_total_show, 'center', 13*h/20, gray);
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                    Screen('DrawTexture', wPtr, t2, [], deck_A);
                ...
                elseif key_selected == KbName('l') && isempty(c2)       
                    Current_total(deck_jump+1,1) = Current_total(deck_jump,1);
                    Previous_total (deck_jump+1,1)= Previous_total (deck_jump,1);
                    Current_total_show = ['Current total: $' num2str(Current_total(deck_jump+1,1))];
                    Previous_total_show = ['Previous total: $' num2str(Previous_total (deck_jump+1,1))];
                    ...
                    Screen('DrawText', wPtr, 'Deck C', w/2+w/16+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'Deck D', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'PASS', w/2-w/16-deck_width+deck_width/10, h/5+deck_height+4*h/50,[255 0 0]);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Current_total_show, 'center', 12*h/20, 0);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Previous_total_show, 'center', 13*h/20, gray);
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                    Screen('DrawTexture', wPtr, t1, [], deck_A);
                ...
                elseif key_selected == KbName('l') && isempty(c3)
                    Current_total(deck_jump+1,1) = Current_total(deck_jump,1);
                    Previous_total (deck_jump+1,1)= Previous_total (deck_jump,1);
                    Current_total_show = ['Current total: $' num2str(Current_total(deck_jump+1,1))];
                    Previous_total_show = ['Previous total: $' num2str(Previous_total (deck_jump+1,1))];
                    ...
                    Screen('DrawText', wPtr, 'Deck C', w/2+w/16+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'Deck D', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'PASS', w/2+w/16+deck_width/10, h/5+deck_height+4*h/50,[255 0 0]);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Current_total_show, 'center', 12*h/20, 0);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Previous_total_show, 'center', 13*h/20, gray);
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                    Screen('DrawTexture', wPtr, t1, [], deck_A);
                ...
                elseif  key_selected == KbName('l') && isempty(c4)
                    Current_total(deck_jump+1,1) = Current_total(deck_jump,1);
                    Previous_total (deck_jump+1,1)= Previous_total (deck_jump,1);
                    Current_total_show = ['Current total: $' num2str(Current_total(deck_jump+1,1))];
                    Previous_total_show = ['Previous total: $' num2str(Previous_total (deck_jump+1,1))];
                    ...
                    Screen('DrawText', wPtr, 'Deck C', w/2+w/16+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'Deck D', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+h/50,0);
                    Screen('DrawText', wPtr, 'PASS', w/2+3*w/16+deck_width+deck_width/10, h/5+deck_height+4*h/50,[255 0 0]);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Current_total_show, 'center', 12*h/20, 0);
                    [nx, ny, bbox] = DrawFormattedText(wPtr,Previous_total_show, 'center', 13*h/20, gray);
                    [nx, ny, bbox] = DrawFormattedText(wPtr, 'Press <SPACEBAR> to Continue', 'center', 11*h/12, [0 0 255]);
                    Screen('DrawTexture', wPtr, t1, [], deck_A);
...
% if no key is pressed during 4s
        ...
        if  isempty(key_selected) && isempty(c1)
            Current_total(deck_jump+1,1) = Current_total(deck_jump,1);
            Previous_total (deck_jump+1,1)= Previous_total (deck_jump,1);
            Current_total_show = ['Current total: $' num2str(Current_total(deck_jump+1,1))];
            Previous_total_show = ['Previous total: $' num2str(Previous_total (deck_jump+1,1))];
        ...
        elseif       isempty(key_selected)  && isempty(c2)
            Current_total(deck_jump+1,1) = Current_total(deck_jump,1);
            Previous_total (deck_jump+1,1)= Previous_total (deck_jump,1);
            Current_total_show = ['Current total: $' num2str(Current_total(deck_jump+1,1))];
            Previous_total_show = ['Previous total: $' num2str(Previous_total (deck_jump+1,1))];
        ...
        elseif       isempty(key_selected) && isempty(c3)
            Current_total(deck_jump+1,1) = Current_total(deck_jump,1);
            Previous_total (deck_jump+1,1)= Previous_total (deck_jump,1);
            Current_total_show = ['Current total: $' num2str(Current_total(deck_jump+1,1))];
            Previous_total_show = ['Previous total: $' num2str(Previous_total (deck_jump+1,1))];
        ...
        elseif   isempty(key_selected) && isempty(c4)
            Current_total(deck_jump+1,1) = Current_total(deck_jump,1);
            Previous_total (deck_jump+1,1)= Previous_total (deck_jump,1);
            Current_total_show = ['Current total: $' num2str(Current_total(deck_jump+1,1))];
            Previous_total_show = ['Previous total: $' num2str(Previous_total (deck_jump+1,1))];
